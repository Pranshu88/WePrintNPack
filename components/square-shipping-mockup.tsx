...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function SquareShippingMockup({ product, onClose, hideFlaps = true }: Props) {
  ...
  async function handleDownloadDieline(fmt: "ai" | "pdf" | "dxf" | "3d" = dielineFmt) {
    ...
    try {
      if (fmt === "dxf") {
        ...
        downloadDXF(buildDielineVectorSvg(node), "square-shipping-box-dieline.dxf");
      } else if (fmt === "ai") {
        const svg = buildDielineVectorSvg(node);
        const svgStr = new XMLSerializer().serializeToString(svg);
        triggerBlobDownload(new Blob([svgStr], { type: "application/postscript" }), "square-shipping-box-dieline.ai");
      } else if (fmt === "3d") {
        const { default: html2canvas } = await import("html2canvas");
        const canvas = await html2canvas(node, { backgroundColor: "#ffffff", scale: 2, useCORS: true });
        triggerBlobDownload(await (await fetch(canvas.toDataURL("image/jpeg", 0.92))).blob(), "square-shipping-box-mockup.jpg");
      } else {
        ...
        doc.save("square-shipping-box-dieline.pdf");
      }
    } catch (err) {
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, background: "#f1f5f9", display: "flex", flexDirection: "column", overflow: "hidden", position: "relative" }}>
          ...
          >
            ...
            <div ref={dielineCanvasRef} style={{ position: "relative", width: dilineCanvasW * zoom, height: dilineCanvasH * zoom, flexShrink: 0 }}>
