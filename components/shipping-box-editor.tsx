...
export default function ShippingBoxEditor({ product, onClose, hideFlaps = false }: Props) {
  ...
  const dielineFaces = (hideFlaps && view === "inside")
    ? faces.map(f => ({ ...f, x: 2 * NX1 + sqPanelW * 4 - f.x - f.w }))
             .sort((a, b) => a.y !== b.y ? a.y - b.y : a.x - b.x)
    : !hideFlaps
          ...
          >
            ...
            <div style={{ position: "relative", width: dilineCanvasW * zoom, height: DIELINE_H_FULL * zoom, flexShrink: 0 }}>
              ...
              {hideFlaps && (() => {
                ...
                const hy = s(sqBaseY + frontFace.h) - s(20);
                const vy1 = s(sqBaseY) + s(10);
                const vy2 = s(sqBaseY + frontFace.h) - s(10);
                const vx  = s(fp.x + fp.w / 2);
                return (
                  <svg style={{ position: "absolute", inset: 0, pointerEvents: "none", zIndex: 10 }} width={(TOTAL_DIELINE_W + SQ_EXTRA_W) * z} height={DIELINE_H_FULL * z}>
                    <g fill="none" stroke="#3b82f6" strokeWidth={s(1.5)}>
                      {/* 305 mm horizontal width */}
                      ...
                      <text x={panelCX} y={hy - s(6)} textAnchor="middle" fill="#3b82f6" stroke="none" fontSize={s(13)} fontWeight="800">305 mm</text>
                      {/* vertical height */}
                      <line x1={vx} y1={vy1} x2={vx} y2={vy2} />
                      <line x1={vx - tk} y1={vy1} x2={vx + tk} y2={vy1} />
                      <line x1={vx - tk} y1={vy2} x2={vx + tk} y2={vy2} />
                      <text x={vx + s(6)} y={(vy1 + vy2) / 2 + s(6)} fill="#3b82f6" stroke="none" fontSize={s(13)} fontWeight="800">305 mm</text>
                    </g>
                  </svg>
                );
              })()}
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  const pW = hideFlaps ? sqPanelW : BW, pD = hideFlaps ? sqPanelW : BD;
                  existing.push({ id: cartId, name: product.name, qty: selectedQty, pricePerUnit: ppu, total: ppu * selectedQty, thumb, boxFaceImages, previewBoxColor, previewW: pW, previewH: BH, previewD: pD });
