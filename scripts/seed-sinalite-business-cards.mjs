/**
 * Imports Sinalite's "Business Cards" category products as new Gallery Templates
 * under the existing "premium-business-cards" product, without touching the
 * 3 existing hand-built templates (Business Cards, Premium Business Cards,
 * Luxury Business Cards) — those keep their current seed designs untouched.
 *
 * Run: node --env-file=.env.local ./scripts/seed-sinalite-business-cards.mjs
 */
...
const PROTECTED_NAMES = new Set(["Business Cards", "Premium Business Cards", "Luxury Business Cards"]);
...
let _c = 0;
function uid() { return `sn${String(++_c).padStart(4, "0")}${Date.now().toString(36)}`; }
...
async function main() {
  ...
  const businessCards = products.filter(
    (p) => p.category === "Business Cards" && p.enabled === 1 && !PROTECTED_NAMES.has(p.name.trim())
  );
  console.log(`Found ${businessCards.length} Sinalite "Business Cards" products to import (excluding the 3 protected templates).`);
  ...
  const existingIds = new Set(
    db.prepare("SELECT sinalite_id FROM gallery_templates WHERE product_slug = ? AND sinalite_id IS NOT NULL")
      .all(PRODUCT_SLUG)
      .map((r) => r.sinalite_id)
  );
