/**
 * Imports Sinalite's "Coroplast Signs & Yard Signs" category products as gallery templates
 * under "yard-signs", then repurposes the first 2 (arr[0], arr[1]) to replace the hand-built
 * "Business Yard Sign" / "Elite Yard Sign" templates — keeping their real Sinalite product
 * name, but tagged with sinalite_id so they keep using the same YARDSIGN_SEED_DESIGNS set and
 * the same $379/$189 pricing as before. All other imported yard signs get no seed-design option.
 *
 * Run: node --env-file=.env.local ./scripts/replace-yardsign-templates-with-sinalite.mjs
 */
...
const OLD_NAMES = ["Business Yard Sign", "Elite Yard Sign"];
const SINALITE_CATEGORIES = new Set(["Coroplast Signs & Yard Signs", "Coroplast Signs & Yard Signs-"]);
...
let _c = 0;
function uid() { return `snys${String(++_c).padStart(4, "0")}${Date.now().toString(36)}`; }

// The 2 tiers to keep seed-designs for, mapped to their original price/specs copy.
const TIER_DEFAULTS_BY_ORDER = [
  { price: "$379 + tax", specs: ['Size: 18" × 24"', "4mm coroplast", "H-stakes included"] },
  { price: "$189 + tax", specs: ['Size: 18" × 24"', "4mm coroplast", "Full colour", "H-stakes included"] },
];
...
async function main() {
  ...
  const yardSigns = products.filter((p) => SINALITE_CATEGORIES.has(p.category) && p.enabled === 1 && !OLD_NAMES.includes(p.name.trim()));
  console.log(`Found ${yardSigns.length} Sinalite "Coroplast Signs & Yard Signs" category products.`);
  ...
  // Replace the 2 hand-built tiers with the first 2 newly-imported Sinalite yard signs (arr[0], arr[1]).
