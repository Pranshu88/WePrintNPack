/**
 * Imports Sinalite's "X-Frame Banners" category products as new gallery templates under
 * "x-frame-banners" — a brand-new category with no prior hand-built templates, so this is
 * a plain import.
 *
 * Run: node --env-file=.env.local ./scripts/seed-sinalite-xframe-banners.mjs
 */
...
const PRODUCT_SLUG = "x-frame-banners";
...
let _c = 0;
function uid() { return `snxf${String(++_c).padStart(4, "0")}${Date.now().toString(36)}`; }
...
async function main() {
  ...
  const items = products.filter((p) => p.category === "X-Frame Banners" && p.enabled === 1);
  console.log(`Found ${items.length} Sinalite "X-Frame Banners" category products.`);
