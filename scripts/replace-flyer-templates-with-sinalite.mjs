/**
 * Imports Sinalite's "Flyers" category products as gallery templates under "bold-flyers",
 * then repurposes the first 2 (arr[0], arr[1]) to replace the hand-built "Express Flyers" /
 * "Prime Flyers" templates — keeping their real Sinalite product name, but tagged with
 * sinalite_id so they keep using the same FLYER_SEED_DESIGNS seed-design set and the same
 * $159/$329 pricing as before. All other imported flyers get no seed-design option.
 *
 * Run: node --env-file=.env.local ./scripts/replace-flyer-templates-with-sinalite.mjs
 */
...
const PRODUCT_SLUG = "bold-flyers";
const OLD_NAMES = ["Express Flyers", "Prime Flyers"];
...
let _c = 0;
function uid() { return `snf${String(++_c).padStart(4, "0")}${Date.now().toString(36)}`; }

// The 2 tiers to keep seed-designs for, mapped to their original price/specs copy.
const TIER_DEFAULTS_BY_ORDER = [
  { price: "$159 + tax", specs: ['Size: 8.5" × 5.5"', "100lb Gloss Text", "Full colour", "Double-sided"] },
  { price: "$329 + tax", specs: ['Size: 8.5" × 11"', "100lb Gloss Text", "Full colour", "Double-sided"] },
];
...
async function main() {
  ...
  const flyers = products.filter((p) => p.category === "Flyers" && p.enabled === 1 && !OLD_NAMES.includes(p.name.trim()));
  console.log(`Found ${flyers.length} Sinalite "Flyers" category products.`);
  ...
  // Replace the 2 hand-built tiers with the first 2 newly-imported Sinalite flyers (arr[0], arr[1]).
