/**
 * Imports Sinalite's "Square Cut Labels / Stickers" category products as new gallery templates
 * under "square-cut-labels-stickers" — a brand-new category with no prior hand-built templates,
 * so this is a plain import.
 *
 * Run: node --env-file=.env.local ./scripts/seed-sinalite-square-cut-labels.mjs
 */
...
const PRODUCT_SLUG = "square-cut-labels-stickers";
...
let _c = 0;
function uid() { return `snscl${String(++_c).padStart(4, "0")}${Date.now().toString(36)}`; }
...
async function main() {
  ...
  const items = products.filter((p) => p.category === "Square Cut Labels / Stickers" && p.enabled === 1);
  console.log(`Found ${items.length} Sinalite "Square Cut Labels / Stickers" category products.`);
