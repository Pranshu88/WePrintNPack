/**
 * Imports Sinalite's "Large Format Posters" category products as new gallery templates
 * under the "large-format-posters" product slug (a brand-new category with no prior
 * hand-built templates, so this is a plain import — no tier/seed-design replacement needed).
 *
 * Run: node --env-file=.env.local ./scripts/seed-sinalite-large-format-posters.mjs
 */
...
const PRODUCT_SLUG = "large-format-posters";
...
let _c = 0;
function uid() { return `snlfp${String(++_c).padStart(4, "0")}${Date.now().toString(36)}`; }
...
async function main() {
  ...
  const items = products.filter((p) => p.category === "Large Format Posters" && p.enabled === 1);
  console.log(`Found ${items.length} Sinalite "Large Format Posters" category products.`);
